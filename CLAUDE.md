...
4. **머지되면 그 자리에서 바로 `dev` 를 따라 올린다. 새 작업보다 먼저.**
   ...
   **먼저 하라는 게 핵심이다.** 당기기 전에 `dev` 에 커밋하면 두 브랜치가
   갈라져 `--ff-only` 가 실패한다. 그때는 일반 머지로 합칠 수밖에 없고,
   `dev` 에 머지 커밋이 하나 남는다.

   ```
   git merge origin/main       # 갈라진 뒤에만. 히스토리가 한 번 갈라졌다 붙는다
   ```
