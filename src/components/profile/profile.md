...
### 개념
방문자가 나(Mr. Chu)를 알아갈수록 코인이 채워지는 구조.
"프로필 탐험" → "코인 획득" → "코인 게이지 시각화"

### 코인 획득 조건 (예정)
| 액션 | 코인 |
|------|------|
| LinkedIn 포스트 5초 이상 읽기 | +10 |
| 인벤토리 아이템 클릭/배치 | +5 |
| 카메라 모드 전환 | +3 |
| (추후) 섹션 방문 | +? |

### 저장
- `useCoinStore` (zustand + persist → localStorage)
- key: `coin-store`
- 중복 지급 방지: 액션 ID 기반 `earned: Record<string, boolean>`

### 표시
- 화면 어딘가에 코인 게이지 또는 카운터
- 가득 차면 특별 리액션? (추후)

### 연동
- `usePostViewStore` → 포스트 읽음 처리 시 코인 지급
- 인벤토리 이벤트 → 코인 지급
