...
import styles from './ProfileDragDropManager.module.scss'
...
export const useProfileDragDropManager = () => React.useContext(ProfileDragDropContext)
...
export const ProfileDragDropManager: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const startThumbnailDrag = React.useCallback(
    (code: string, x: number, y: number) => {
      ...
      const onPointerUp = () => { cleanup(); endDrag() }
      const onPointerCancel = () => { cleanup(); cancelDrag() }
  ...
  const contextValue = React.useMemo(() => ({ startThumbnailDrag }), [startThumbnailDrag])
    ...
    <ProfileDragDropContext.Provider value={contextValue}>
      ...
      {preview && itemDef && (
        <div
          ...
          style={{ left: preview.x - 22, top: preview.y - 22, background: itemDef.color }}
        />
      )}
