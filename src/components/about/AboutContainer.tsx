...
const AboutContainer = () => {
  ...
  useEffect(() => {
    ...
    function onHandlScroll() {
      const scrollPosition = rightContainerRef.current
        ? rightContainerRef.current.scrollTop + 440
        : 0;
      const sectionPositions = categoryRefs.current.map((ref) =>
        ref ? ref.offsetTop : 0
      );
      ...
      for (let i = 0; i < sectionPositions.length; i++) {
        ...
        if (
          ...
          scrollPosition < (sectionPositions[nextIndex] || Infinity)
        ) {
          setSection(i);
          autoChange();
          break;
